...
namespace Pansiyon1
{
    public partial class FrmYeniMusteri : Form
    {
        ...
        {
            MessageBox.Show("Yeşil Renkli bloklar Boş odaları gösterir");
        }

        private void BtnDoluOda_Click(object sender, EventArgs e)
        {
            MessageBox.Show("Kırmızı Renkli bloklar Dolu odaları gösterir");
        }
        ...
        {
            baglanti.Open();
            SqlCommand komut = new SqlCommand("insert into MusteriEkle (Adi,Soyadi,Cinsiyet ,Telefon,Mail,TC,OdaNo,Ucret,GirisTarihi,CikisTarihi) values('" + TxtAdi.Text + "' , '"  + TxtSoyad.Text+ "' , '" +CmbCinsiyet.Text + "' , '" + MskTelefon.Text + "' , '" +TxtMail.Text + "' , '" + TxtTcNo.Text + "' , '" + TxtOdaNumarasi.Text + "' , '" + TxtUcret.Text + "' , '" + DtpGirisTarihi.Value.ToString("yyyy-MM-dd")+ "' , '" +DtpCikisTarihi.Value.ToString("yyyy-MM-dd") +  "' ) ", baglanti);
            komut.ExecuteNonQuery();
            baglanti.Close();
